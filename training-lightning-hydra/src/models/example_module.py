...
class ExampleLitModule(LightningModule):
    ...
    def __init__(
        ...
        # Store primitive hparams only (avoid saving complex objects like partials/OmegaConf
        # types that break PyTorch 2.6+ torch.load(weights_only=True) checkpoint loading).
        # We reconstruct optimizer/scheduler from these in configure_optimizers.
        opt_cfg = optimizer
        sched_cfg = scheduler
        ...
        # Store config refs (not hparams) for reconstructing in configure_optimizers
        self._optimizer_cfg = opt_cfg
        self._scheduler_cfg = sched_cfg
